...
| --- | --- | --- |
| `<section>` | **Section**, a coloured band that groups blocks | `data-background="none\|soft\|tint\|dark"`, `data-spacing="sm\|md\|lg"` |
| `<h1>` … `<h6>` | **Heading**. `h4`–`h6` become the smallest size | `data-align="left\|center\|right"` |
| `<p>`, `<blockquote>` | Paragraph of a **Text** block | `data-align="left\|center\|right"` |
| `<ul>`, `<ol>` | Bullet list in a **Text** block | none. Numbering is not kept |
| `<a href="…">Label</a>` | **Button** | `data-variant="solid\|outline"`, `data-align`, `data-page="slug"` |
| `<img alt="…">` | **Image** placeholder | `data-width="20-100"`, `data-align`, `data-corners="rounded\|square"` |
| `<hr>` | **Divider** | `data-spacing="sm\|md\|lg"` |
...
- **Section backgrounds become colours.** `data-background` still takes `none`, `soft`, `tint` or `dark`, which mean no fill, `#f4f6f8`, `#ccfbf1` and `#0f172a`. In the editor the person sees an ordinary colour they can change with the colour picker; there is no preset dropdown any more. Text on a dark fill turns light by itself.
- **Section and Columns nest one way.** A Section can hold Columns; nothing holds a Section. A `<section>` inside another `<section>` is flattened into its parent. A column holds Heading, Text, Image, Button, Divider and Spacer only: nested `data-columns` is dropped. Only the first three children of a `data-columns` element count.
...
Count blocks like this: each Section, Columns, Heading, Text run, Image, Button, Divider and Spacer is one. `launch.html` has 53.
...
- **Never invent facts.** Names, prices, addresses, phone numbers, hours, statistics and testimonials get a bracketed placeholder: `[Company name]`, `[opening hours]`, `[Price]`. Generic sample content that isn't a fact (dish names, feature descriptions) is fine.
